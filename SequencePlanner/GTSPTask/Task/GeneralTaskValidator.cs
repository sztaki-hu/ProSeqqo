...
namespace SequencePlanner.GTSPTask.Task.General
{
    public class GeneralTaskValidator
    {
        ...
        private static void CheckMotionPrecedence(GeneralTask task)
        {
            ...
            SeqLogger.Debug("MotionPrecedence: " + task.ProcessPrecedence.Count + " precedences", nameof(GeneralTaskValidator));
        }
        ...
        private static void CheckPositionMatrix(PositionMatrix positionMatrix)
        {
            ...
            SeqLogger.Trace("CheckPositionMatrix validated!", nameof(GeneralTaskValidator));
        }
        ...
        private static void CheckConfigList(GeneralTask task)
        {
            ...
            SeqLogger.Debug("ConfigList: " + task.PositionMatrix.Positions.Count, nameof(GeneralTaskValidator));
        }

        //Q10
        private static void CheckLocalSearchStrategy(GeneralTask task)
        {
            SeqLogger.Debug("LocalSearchStrategy: " + task.LocalSearchStrategy.ToString(), nameof(GeneralTaskValidator));
        }
        ...
        {
            SeqLogger.Debug("UseMIPPrecedenceSolver: " + task.UseMIPprecedenceSolver, nameof(GeneralTaskValidator));
        }
        ...
        private static void CheckTimeLimit(GeneralTask task)
        {
            if (task.TimeLimit <= 0)
            {
                SeqLogger.Debug("TimeLimit: 0 - Automatic based on solver", nameof(GeneralTaskValidator));
                SeqLogger.Warning("Time limit not given, running can take a long time", nameof(GeneralTaskValidator));
            }

            if (task.TimeLimit > 0)
                SeqLogger.Debug("TimeLimit: " + task.TimeLimit + "ms - " + System.TimeSpan.FromMilliseconds(task.TimeLimit).ToString(), nameof(GeneralTaskValidator));
        ...
        //Q5
        private static void CheckWeightMultiplier(GeneralTask task)
        {
            if (task.WeightMultipier == 0)
                throw new SeqException("WeightMultipier should be greater then 0.");
            SeqLogger.Debug("WeightMultipier: " + task.WeightMultipier, nameof(GeneralTaskValidator));
        }
        ...
        private static void CheckCycle(GeneralTask task)
        {
            ...
            SeqLogger.Trace("CheckCycle validated!", nameof(GeneralTaskValidator));
        }
        ...
        private static void CheckDistaceFunction(GeneralTask task)
        {
            ...
            SeqLogger.Debug("DistanceFunction:" + task.PositionMatrix.DistanceFunction.FunctionName, nameof(GeneralTaskValidator));
        ...
        private static void CheckTaskType(GeneralTask task)
        {
            ...
            if (task.MotionPrecedence.Count < 1)
                ...
                SeqLogger.Debug(task.MotionPrecedence.Count + " position precedence found.", nameof(GeneralTaskValidator));
